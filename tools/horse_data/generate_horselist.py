...
from tools.horse_data.site_metadata import (
    ...
    default_site_metadata_for_source,
    default_site_metadata_for_workbook,
    load_site_metadata,
)
...
def normalize_text_list(value: object) -> list[str]:
    if not isinstance(value, list):
        return []
    return [item_text for item in value if (item_text := text(item).strip())]


def normalize_skill_tabs(value: object) -> list[dict[str, object]]:
    if not isinstance(value, list):
        return []
        ...
        tabs.append(
            {
                ...
                "effects": normalize_text_list(item.get("effects")),
                "conditions": normalize_text_list(item.get("conditions")),
                "targets": normalize_text_list(item.get("targets")),
                "probability": normalize_text_list(item.get("probability")),
            }
        )
    return tabs


def build_skill_card_payload(value: object) -> dict[str, object] | None:
    if not isinstance(value, dict):
        return None
    name = text(value.get("name")).strip()
    description = normalize_text_list(value.get("description"))
    detail_url = text(value.get("detailUrl")).strip()
    detail_tabs = normalize_skill_tabs(value.get("detailTabs"))
    if not name and not description and not detail_url and not detail_tabs:
        return None
    return {
        "name": name,
...
def build_records_from_source(
    ...
    for row in iter_all_rows(source[SOURCE_ALL_KEY]):
        ...
        temperament_data = build_skill_card_payload(
            horse_site_metadata.get("innateTalent")
        )
        ability_name = (
            text(ability_data.get("name")).strip()
        ...
        record = {
            ...
            "card": {
                ...
                "temperamentData": temperament_data,
...
def build_records(
    ...
    return build_records_from_source(
        ...
        load_site_metadata(site_metadata_path or default_site_metadata_for_workbook(workbook_path)),
    )
...
def build_records_from_json(
    ...
    return build_records_from_source(
        ...
        load_site_metadata(site_metadata_path or default_site_metadata_for_source(source_json_path)),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional site metadata JSON with extraordinary ability / innate talent details. "
            f"Defaults to {DEFAULT_SITE_METADATA_PATH} when available."
        ),
    )
    parser.add_argument(
        "--output",
